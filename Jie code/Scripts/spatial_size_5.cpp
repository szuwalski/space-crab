...
Type objective_function<Type>::operator() ()
{
    ...
    for(int k=0; k<n_k; k++){
        for(int p=0; p<n_p; p++){
            dpred1_pkt(p,k,0) = log(exp(phi_p(p)) + exp(logR_mu(0))*R_sex(0)*R_size(p)); /////// male - 1; female -2
            dpred2_pkt(p,k,0) = log(exp(phi_p(p)) + exp(logR_mu(0))*R_sex(1)*R_size(p));
        }}
    ...
    for(int k=0; k<n_k; k++){
        ...
        if( (t+1)<n_t ){
            dpred1_pkt.col(t+1).col(k) = log(dhat1_pkt.col(t).col(k) + exp(logR_mu(t+1))*R_sex(0)*R_size);
            dpred2_pkt.col(t+1).col(k) = log(dhat2_pkt.col(t).col(k) + exp(logR_mu(t+1))*R_sex(1)*R_size);
        }
