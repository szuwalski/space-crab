...
Type objective_function<Type>::operator() ()
{
    ...
    vector<Type> dimmat_p(n_p);
    vector<Type> dmat_p(n_p);   
    array<Type> dhat_pkt(n_p, n_k, n_t);       // 
    array<Type> dpred_pkt(n_p, n_k, n_t);      // Predicted density for each year
    ...
    for(int k=0; k<n_k; k++){
        for(int p=0; p<n_r; p++){
            dpred_pkt(p,k,0) = log(exp(logR_mu(0))*R_size(p));
        }
        for(int p=n_r; p<n_p; p++){
            dpred_pkt(p,k,0) = phi_p(p-n_r);
        }
    }
    ...
    for(int k=0; k<n_k; k++){
        ...
        for (int p=0; p<n_p; p++){
            ...
            dimmat_p(p) = exp(d_pkt(p,k,t) - M_pkt(p,k,t) - F_male_pkt(p,k,t)) * immature(p);
            dmat_p(p) = exp(d_pkt(p,k,t) - M_pkt(p,k,t) - F_male_pkt(p,k,t)) * mature(p);
        }
        
        tmp1_p = growthmale_tran * dimmat_p;
        
        for (int p=0; p<n_p; p++){
            dhat_pkt(p,k,t) = tmp1_p(p) + dmat_p(p);
            chat_pkt(p,k,t)  = a_k(k) * (1 - exp( -F_male_pkt(p,k,t) )) * exp(d_pkt(p,k,t) -M_frac * M_pkt(p,k,t));
        }
        
        if( (t+1)<n_t ){
            dpred_pkt.col(t+1).col(k) = log(dhat_pkt.col(t).col(k) + exp(logR_mu(t+1))*R_size);
        }
