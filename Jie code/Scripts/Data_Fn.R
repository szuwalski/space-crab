...
#' @export
Data_Fn = function(Version, obsmodel_p=NULL, b_i, s_i, t_i, p_i, a_x, n_r, R_sex, R_size, M_frac, mature, immature, size_midpoints, M_pkt=NULL, growthmale_tran, growthfemale_tran, MeshList, n_factors=1, c_pkt=NULL, catchlogsd, CheckForErrors=TRUE ){
  ...
  # Fill in defaults
  if( is.null(c_pkt)) c_pkt = array(0, dim=c(length(unique(data_frame[,'size_class']))/2,MeshList$anisotropic_mesh$n,diff(range(data_frame[,'year']))+1) )
  if( is.null(M_pkt)) M_pkt = array(0.23, dim=c(length(unique(data_frame[,'size_class'])),MeshList$anisotropic_mesh$n,diff(range(data_frame[,'year']))+1) )
 
  if( Version%in%c("spatial_size_6")){
  ...
  if( Version%in%c("spatial_size_1","spatial_size_2","spatial_size_3","spatial_size_4","spatial_size_5","spatial_size_6","spatial_size_7")){
    ...
    if(Version%in%c("spatial_size_1","spatial_size_2","spatial_size_3","spatial_size_4","spatial_size_5")) Return = list("Options_vec"=options_vec, "ObsModel_p"=obsmodel_p, "n_i"=nrow(data_frame), "n_s"=length(unique(data_frame[,'sitenum'])), "n_t"=diff(range(data_frame[,'year']))+1, "n_k"=MeshList$anisotropic_mesh$n, "n_p"=length(unique(data_frame[,'size_class']))/2, "n_j"=n_factors, "b_i"=data_frame[,'catch'], "p_i"=as.numeric(data_frame[,'size_class'])-1, "s_i"=data_frame[,'sitenum']-1, "t_i"=data_frame[,'year']-min(data_frame[,'year']), "R_sex"=R_sex, "R_size"=R_size, "M_frac"=M_frac, "immature"=immature, "mature"=mature,"M_pkt"=M_pkt, "growthmale_tran"=growthmale_tran, "growthfemale_tran"=growthfemale_tran, "c_pkt"=c_pkt, "catchlogsd"=catchlogsd, "a_k"=a_k, "spde"=NULL, "M0"=M0, "M1"=M1, "M2"=M2)
    if(Version%in%c("spatial_size_6")) Return = list("Options_vec"=options_vec, "ObsModel_p"=obsmodel_p, "n_i"=nrow(data_frame), "n_s"=length(unique(data_frame[,'sitenum'])), "n_t"=diff(range(data_frame[,'year']))+1, "n_k"=MeshList$anisotropic_mesh$n, "n_p"=length(unique(data_frame[,'size_class'])), "n_j"=n_factors, "n_r"=n_r, "b_i"=data_frame[,'catch'], "p_i"=as.numeric(data_frame[,'size_class'])-1, "s_i"=data_frame[,'sitenum']-1, "t_i"=data_frame[,'year']-min(data_frame[,'year']), "R_size"=R_size, "M_frac"=M_frac, "immature"=immature, "mature"=mature,"M_pkt"=M_pkt, "growthmale_tran"=growthmale_tran, "c_pkt"=c_pkt, "catchlogsd"=catchlogsd, "a_k"=a_k, "spde"=NULL, "M0"=M0, "M1"=M1, "M2"=M2)
    if(Version%in%c("spatial_size_7")) Return = list("Options_vec"=options_vec, "ObsModel_p"=obsmodel_p, "n_i"=nrow(data_frame), "n_s"=length(unique(data_frame[,'sitenum'])), "n_t"=diff(range(data_frame[,'year']))+1, "n_k"=MeshList$anisotropic_mesh$n, "n_p"=length(unique(data_frame[,'size_class'])), "n_j"=n_factors, "n_r"=n_r, "b_i"=data_frame[,'catch'], "p_i"=as.numeric(data_frame[,'size_class'])-1, "s_i"=data_frame[,'sitenum']-1, "t_i"=data_frame[,'year']-min(data_frame[,'year']), "R_size"=R_size, "M_frac"=M_frac, "immature"=immature, "mature"=mature, "size_midpoints"=size_midpoints, "M_pkt"=M_pkt, "growthmale_tran"=growthmale_tran, "c_pkt"=c_pkt, "catchlogsd"=catchlogsd, "a_k"=a_k, "spde"=NULL, "M0"=M0, "M1"=M1, "M2"=M2)
