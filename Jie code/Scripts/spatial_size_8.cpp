...
Type objective_function<Type>::operator() ()
{
    ...
    array<Type> dhat1_pkt(n_p, n_k, n_t);       // density
    array<Type> dpred1_pkt(n_p, n_k, n_t);      // Predicted density for each year
    array<Type> dpred_pkt(n_p, n_k, n_t);      // 
    array<Type> F_male_pkt(n_p, n_k, n_t);        // F
    array<Type> chat_pkt( n_p, n_k, n_t );      // Predicted catch-at-size for each year
    array<Type> ssb_pkt( n_p, n_k, n_t );      // Predicted ssb
    ...
    for(int k=0; k<n_k; k++){
        ...
        for (int p=0; p<n_p; p++){
            ...
            tmp1a_p(p) = exp(d1_pkt(p,k,t)) * exp(-M_pkt(p,k,t)-F_male_pkt(p,k,t));
            tmp2_p(p) = exp(d1_pkt(p,k,t)) * exp(-M_pkt(p,k,t)-F_male_pkt(p,k,t)) * mature(p); //matured N
        }
        
        tmp1_p = growthmale_tran * tmp1a_p;
        
        for (int p=0; p<n_p; p++){
            ssb_pkt(p,k,t) = tmp2_p(p);
            dhat1_pkt(p,k,t) = tmp1_p(p);
            chat_pkt(p,k,t)  = a_k(k) * (1 - exp( -M_pkt(p,k,t)-F_male_pkt(p,k,t) )) * exp(d1_pkt(p,k,t)) * ( F_male_pkt(p,k,t)/( M_pkt(p,k,t) + F_male_pkt(p,k,t)) );
        }
