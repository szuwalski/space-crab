...
Type objective_function<Type>::operator() ()
{
    ...
    PARAMETER_VECTOR(L_var);    // diag variance in covariance matrix
    ...
    PARAMETER_VECTOR (select); // selectivity
    ...
    matrix<Type> Diag_pp(n_p*2, n_p*2);
    Diag_pp.setZero();
    Cov_pp.setZero();
    int Count = 0;
    ...
    if( Options_vec(0)==0 ){
        ...
        Cov_pp = L_pj*L_jp + Type(0.000001)*Identity_pp + Diag_pp;  // additive constant to make Cov_pp invertible
    }
    ...
    for(int k=0; k<n_k; k++){
        for(int p=0; p<n_p; p++){
            dpred1_pkt(p,k,0) = log(exp(phi_p(p)) + exp(logR_kt(k,0))*R_sex(0)*R_size(p)); /////// male - 1; female -2
            dpred2_pkt(p,k,0) = log(exp(phi_p(p)) + exp(logR_kt(k,0))*R_sex(1)*R_size(p));
        }}
    ...
    for(int k=0; k<n_k; k++){
        ...
        for (int p=0; p<n_p; p++){
            F_male_pkt(p,k,t) = exp(logF_male_kt(k,t)) * select(p);
            ...
            tmp3a_p(p) = exp(d2_pkt(p,k,t)) * exp(-M_pkt(p,k,t)) * immature(p);
            tmp4_p(p) = exp(d2_pkt(p,k,t)) * exp(-M_pkt(p,k,t)) * mature(p);
            
            //tmp3a_p(p) = exp(d2_pkt(p,k,t)) * exp(-M_pkt(p,k,t)-F_female_pkt(p,k,t)) * immature(p);
            //tmp4_p(p) = exp(d2_pkt(p,k,t)) * exp(-M_pkt(p,k,t)-F_female_pkt(p,k,t)) * mature(p);
        }
        ...
        for (int p=0; p<n_p; p++){
            ...
            dhat2_pkt(p,k,t) = tmp3_p(p) + tmp4_p(p);
            chat_pkt(p,k,t)  = a_k(k) * (1 - exp( -F_male_pkt(p,k,t) )) * exp(d1_pkt(p,k,t)) * exp( -M_frac * M_pkt(p,k,t) );
        }
        
        if( (t+1)<n_t ){
            dpred1_pkt.col(t+1).col(k) = log(dhat1_pkt.col(t).col(k) + exp(logR_kt(k,t+1))*R_sex(0)*R_size);
            dpred2_pkt.col(t+1).col(k) = log(dhat2_pkt.col(t).col(k) + exp(logR_kt(k,t+1))*R_sex(1)*R_size);
        }
