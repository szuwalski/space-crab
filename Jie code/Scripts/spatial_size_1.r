...
source('/Users/jiecao/Desktop/UW_work/model_devp/SSST/Codes/Data_Fn.r')
source('/Users/jiecao/Desktop/UW_work/model_devp/SSST/Codes/Build_TMB_Fn.r')
Version = c("spatial_size_1","spatial_size_2","spatial_size_3","spatial_size_4","spatial_size_5",
            "spatial_size_6","spatial_size_7")[7]

n_x = c(30, 50, 75, 100, 150, 200, 300)[4] # Number of stations
...
growthmale_tran = growth_trans(15,1.0,0.1,low_s,up_s,bin_width)
#growthmale_tran = growth_trans(7.4,1.5,0.75,low_s,up_s,bin_width)
...
if(determ){
  ...
  DateFile = paste0('/Users/jiecao/Desktop/UW_work/model_devp/SSST','/SSST_output/','iterations')
}
dir.create(DateFile)

DF = read.csv('/Users/jiecao/Desktop/UW_work/model_devp/SSST/Simulation_Data/survey_data.csv')
...
ObsModel_p = rep( switch(ObsModel,"Poisson"=0, "Lognormal"=1, "ZILN"=2, "LNP"=3, "Normal"=4), length(unique(Data_Geostat[,'Size_class'])) )
TmbData = Data_Fn( "Version"=Version, "obsmodel_p"=ObsModel_p, "b_i"=Data_Geostat[,'Catch_N'], "c_pkt"=c_pkt, "s_i"=Data_Geostat[,'knot_i'], "t_i"=Data_Geostat[,'Year'], "p_i"=Data_Geostat[,'Size_class'], "a_x"=Spatial_List$a_xl[,1], "n_r"=n_r, "R_sex"=R_sex, "R_size"=R_size, "M_frac"=M_frac, "mature"=mature, "immature"=immature,"size_midpoints"=size_midpoints,"M_pkt"=NULL, "growthmale_tran"=growthmale_tran, "growthfemale_tran"=growthfemale_tran, "MeshList"=Spatial_List$MeshList, "n_factors"=Nfactors_est, catchlogsd=catchlogsd )
...
#obj = normalize( obj=Obj, flag="include_data", value=FALSE )
...
par(mfrow=c(1,5))
min=min(catch_pt_ob);max=max(catch_pt_ob)
for (k in 1:n_p){
  ind = 2*n_t*n_p
...
breakpoints <- c(0,5000,6000,7000,10000,12500,14000,15000,16000,17500,20000,30000,50000,100000)

for (t in 1:n_t){
  png(paste('catch','_year-',t,'.png',sep=''), height = 8, width = 6, units = 'in', res=600)
  par(mfrow=c(n_p,2))
  par(mar = c(0, 0, 0, 0), oma = c(1, 4, 4, 0.5))
  par(tcl = -0.25)
  par(mgp = c(2, 0.6, 0))
  for (p in 1:n_p){
    ...
    #mtext(paste('Obs',sep=''),1,-2,adj=0.1,cex=0.8)
    if(p==1){
      mtext(paste('Obs'),3,1)
    }
    mtext(paste('Size class ',p,sep=''),2,0.5,cex=0.9)
